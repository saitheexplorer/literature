...
export default class PlayerMenu extends React.Component {
  ...
  render() {
    if (!this.state.isDeclaring && !this.state.isAsking)
      return (
        <div>
    ...
    let opponents = Range(2, this.props.numberOfPlayers + 1, 2).map(player => (
    ...
    let askableCards = this.props.cardsInPlay
      ...
      .map(card => (
        <option key={card.get('card')} value={card.get('card')}>
          {card.get('card')}
        </option>
      ));
