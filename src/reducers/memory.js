  ...
  switch (action.type) {
    case Constants.START_GAME:
      return state.map(card =>
        card.set(
          'possibleOwners',
          Range(1, action.numberOfPlayers + 1)
            .toList()
            .map(String)
        )
      );
    ...
    case Constants.TRANSFER_CARD:
      const transferCardIndex = state.findIndex(
        card => card.get('card') === action.askedCard
      );

      return state
        .setIn([transferCardIndex, 'owner'], action.askingPlayer)
        .setIn([transferCardIndex, 'possibleOwners'], List());

    case Constants.ASK_QUESTION:
      const askedCardIndex = state.findIndex(
        card => card.get('id') === action.askedCard
      );
      ...
      return state
        .set(
          askedCardIndex,
          removePlayerFromPossibleOwners(action.askingPlayer)(askedCard)
        )
        .map(simplifyPossibleOwners);

    case Constants.DENY_QUESTION:
      const deniedCardIndex = state.findIndex(
        card => card.get('id') === action.askedCard
      );
      ...
      return state
        .set(
          deniedCardIndex,
          removePlayerFromPossibleOwners(action.askedPlayer)(deniedCard)
        )
        .map(simplifyPossibleOwners);
