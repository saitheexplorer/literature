...
export default function askPlayer(askingPlayer, askedPlayer, askedCard) {
  return (dispatch, getState) => {
    ...
    const cardIndex = getState().cardsInPlay.findIndex(
      card =>
        card.get('card') === askedCard && card.get('owner') === askedPlayer
    );

    if (cardIndex !== -1)
      dispatch(transferCard(askingPlayer, cardIndex, askedCard));
    else {
